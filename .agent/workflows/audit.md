...
4. **レポート (Risk Report & Evidence)**
   - Critical: 直ちに修正が必要（脆弱性、Secret漏洩）
   - Warning: リリース前に対処推奨（ライセンス不明、PII疑惑）
   
   **[Evidence Creation]**
   以下のコマンドで、監査結果を物理ファイルとして保存する（銀行/監査対応）。
   Run: `mkdir -p docs/audit_reports`
   Run: `echo "# Audit Report ($(date +%Y-%m-%d))\n\n## Profile: $(cat .agent/config.json)\n\n## Results\n(Pasete Results Here)" > docs/audit_reports/last_audit.md`
   「監査レポートを `docs/audit_reports/` に保存しました。」と報告する。
